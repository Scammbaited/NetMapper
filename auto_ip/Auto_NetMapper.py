...
def get_default_network_range():
    ...
    netmask_cidr = sum([bin(int(x)).count("1") for x in netmask.split(".")])

    network_address = ipaddress.IPv4Network(f"{ip_address}/{netmask_cidr}", strict=False).network_address
    network_range = f"{network_address}/{netmask_cidr}"
...
def create_html(network_map):
    ...
    <body>
        <div class="container">
            <h1 class="mt-5 mb-5 text-center">Network Map</h1>
            {body}
        </div>
    </body>
    </html>
...
if __name__ == "__main__":
    default_network_range = get_default_network_range()
